...
from src.dmdk_handler.gost_xml_transform.gost_xml_transform import GOSTXMLTransform
import base64
...
def sign_ref_element(template):
    ...
    ref_element = template.find(f'.//{{{NSMAP["ns"]}}}RequestData')
    ...
    digest_value = base64.b64encode(bytearray.fromhex(hashedData.Value)).decode()
    ...
    data_signature = signed_data.SignHash(hashedData, Signer.SIGNER, pycades.CADESCOM_CADES_BES, True)
    ...
    signature_value_node.text = data_signature
