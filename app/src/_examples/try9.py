...
class SignedXMLTemplate(XMLTemplate):
    ...
    def _sign_reference(self):
        ...
        hashedData.Algorithm = pycades.CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256
        hashedData.DataEncoding = pycades.CADESCOM_BASE64_TO_BINARY
        hashedData.Hash(base64.b64encode(c14n_signed_info).decode())
    ...
    @staticmethod
    def split_signature(signature, chunk_size=64):
        chunks = []
        for i in range(0, len(signature), chunk_size):
            chunks.append(signature[i:i + chunk_size])
        return '\n'.join(chunks)
